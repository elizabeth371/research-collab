...
HUMAN_SAMPLES = [
    ...
    "比预想的顺利一些。",
    "上午把上周提交的课程论文初稿拿回来, 老师批注了不少地方, 主要集中在"
    "论证的逻辑衔接上。我准备按照批注逐条修改, 先补一段相关工作的对比分析,"
    "再精简结论部分, 周末前把二稿发过去。",
    "实验室新到了一批传感器, 负责设备的同学花了两天时间把数据采集程序调通,"
    "采样率比之前稳定了很多。我借这个机会重新设计了实验方案, 把重复测量的"
    "次数从五次增加到八次, 误差棒应该会明显收窄。",
    "晚饭后把项目的答辩 PPT 大纲列了出来, 一共五页: 背景、方案、实现、"
    "实验和展望。其中实验部分的数据图还没有完全整理好, 明天上午去实验室"
    "把曲线重新导出来, 顺便核对一下测试用例的覆盖率。",
    "早上到实验室先把设备预热起来, 然后花了一个小时把上周的测量数据做了"
    "预处理, 剔除了两组明显异常的点。中午和同学讨论了论文里图表配色的问题,"
    "最后决定统一用蓝灰两色, 既清晰又符合期刊的排版要求。",
]
